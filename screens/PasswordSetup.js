...
const PasswordSetup = ({ navigation, route }) => {
  ...
  //           "Content-Type": "multipart/form-data",
  //         },
  //         timeout: 120000, // Increased to 120 seconds for slow server wake-up
  //         // Uncomment the line below for testing SSL bypass (not for production)
  ...
  //     let errorMsg = "An error occurred while registering. Please try again.";
  //     if (error.response?.data?.message) {
  //       errorMsg = error.response.data.message;
  //     } else if (error.message === "timeout of 120000ms exceeded") {
  //       errorMsg = "Request timed out. Server may be slow or offline.";
  //     } else if (error.message === "Network Error") {
  //       errorMsg =
  //         "Unable to connect to the server. Check your internet or server status.";
  //     }
  ...
  const handleSave = async () => {
    try {
      ...
      console.log("Sending request to server...");
      const response = await axios.post(
        "http://192.168.1.11:4000/api/v1/emp/register",
        formDataToSend,
        {
          headers: {
            "Content-Type": "multipart/form-data",
      ...
      // Check if registration was successful (adjust based on your server's response)
      if (response.data.sucess) {
        // Assuming 'success' is the key; adjust as needed
        await SecureStore.setItemAsync(
          "userData",
          JSON.stringify(response.data.data || response.data) // Fallback if no 'data' key
        );
        Alert.alert("Register Successful", "Welcome back!");
        navigation.navigate("Marketplace");
      } else {
